...
def check():
    month = ['','Baisakh','Jestha','Ashadh','Shrawan','Bhadra','Asoj','Karthik','Mangsir','Poush','Magh','Falgun','Chaitra']
    n1 = int(mon.get())
    x1 = int(yea.get())
    if (n1>=9):
        x = x1-56
    ...
    try:
        ...
        if(n<13 and x<2020):
            ...
            for i in range(len(li_crop)-1):
                val += li_crop[i]+' or '
            val += li_crop[i+1]
            val += '\n\nOn the basis of prediction for '+month[n1]+', '+str(x1)
            val += '\nThe Average temperature is '+str(y1)+'degree celsius and '+'Rainfall is '+str(y2)+'mm.'
